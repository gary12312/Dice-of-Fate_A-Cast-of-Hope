...
namespace DiceFate.Dice
{
    public class KegCylinderVirtual : MonoBehaviour
    {
        ...
        [SerializeField] private float cylinderHeight = 4f;
        [Range(0f, 1f)]
        [SerializeField] private float inwardOffset = 0.2f; // Смещение от края внутрь
        [SerializeField] private GameObject pointToCreate;

        [Header("Movement Speeds")]
        [SerializeField] public float cylinderFollowSpeed = 10f; // Управляется из Mane
        [SerializeField] private float cubesAttractionSpeed = 11f;

        [Header("Cube Physics Settings")]
        [SerializeField] private float minimumDistance = 1f; // Минимальное расстояние между кубами
        [SerializeField] private float pushForce = 2f; // Сила отталкивания

        [Header("Visualization Colors")]
        public Color cylinderColor = Color.yellow;
        public Color detectionColor = new Color(1, 0, 0, 0.3f);
        public Color effectiveRadiusColor = Color.green;
        public Color cubePersonalSpaceColor = Color.cyan;
        ...
        private void HandelDiceDrop(OnDropEvent evt) => DestroyVirtualCylinder();
        ...
        public void MoveСercleToMouse(OnMoveToMouseEvent point)
        {
